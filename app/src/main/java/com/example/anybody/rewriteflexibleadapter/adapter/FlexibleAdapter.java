...
public abstract class FlexibleAdapter<VH extends RecyclerView.ViewHolder,T> extends SelectableAdapter{
    ...
    private void removeRange(Integer integer, int count) {
        if (DEBUG) Log.d(TAG, "removeRange: positionStart :"+integer+"itemCount : " +count);
        for (int i = 0; i < count; ++i) {
            synchronized (_Lock){
                saveDeletedItem(integer,_items.get(integer));
            }
        }
        if (DEBUG) Log.d(TAG, "removeRange: notifyItemRangeRemoved");
        notifyItemRangeRemoved(integer,count);
        updateEmptyView(_items.size());
